...
(= stories* nil comments* nil jobs* nil
...
; Page top
; original sand 246 246 239
(= sand (color 220 180  57) textgray (gray 130))
...
(def alistpage (user t1 label title currentpage (o url label) (o number t))
  ...
  (longpagec user t1 nil label title currentpage url
      ...
      (tag (img src full-logo-url* width 418 height 531 vspace 3 hspace 7)))
                    ...
                    (br 2) (fontcolor black (pr "They form the crest of entrepreneurship that is sweeping across Notre Dame."))
                    (br 4) (fontcolor black (pr "Notre Dame students and alumni inspire to change the world. Our founder himself predicted that this university would be a powerful means for good."))
                    (br 2) (fontcolor black (prbold "We are building a framework of support for every student and alumni who has an idea and the courage to pursue it - the Notre Dame entrepreneurs."))
                    
                    (br 3) (fontcolor black (prbold "We're connecting people"))
                    (br 2) (fontcolor black (pr "who have the drive to create something utterly new. Instead of scattered individuals and under-utilized resources, we're aligning the arrows."))
                    (br 2) (fontcolor black (pr "who can't resist the desire to change the world through the intertwining of creativity and business - in short, through entrepreneurship."))
                    
                    (br 3) (fontcolor black (prbold "Outlined against the same blue, grey"))
...
(newsop jobs () (jobspage user))
       

; Note: dead/deleted items will persist for the remaining life of the 
; cached page.  If this were a prob, could make deletion clear caches.

(newscache jobspage user 1000
...
(def jobstories (user n)
  (retrieve n [cansee user _] jobs*))


(newsop about () (aboutpage user))
...
(def display-job (i s user whence)
  (when (or (cansee user s) (s 'kids))
    (tr (display-item-number i)
        (td (jvotelinks s user whence))
        (titleline s s!url user whence))
    (tr (tag (td colspan (if i 2 1)))    
        (tag (td class 'subtext)
          (hook 'jitemline s user)
          (jitemline s user)
          (editlink s user)
          (unless i (flaglink s user whence))
          (killlink s user whence)
...
(def jdisplay-item-number (i)
  (when i (tag (td align 'right valign 'top class 'title)
            (pr i "."))))

(= follow-threshold* 5)

(def jtitleline (s url user whence)
  (tag (td class (+ "  " 'title))
    (if (cansee user s)
        (do (deadmark s user)
...
(def jvotelinks (i user whence (o downtoo))
  (center
    (if (and (cansee user i)
             (or (no user)
                 (no ((votes user) i!id))))
         (do (jvotelink i user whence 'up)
             (if (and downtoo 
                      (or (admin user)
                          (< (item-age i) downvote-time*))
                      (canvote user i 'down))
                 (do (br)
                     (jvotelink i user whence 'down))
                 ; don't understand why needed, but is, or a new
                 ; page is generated on voting
                 (tag (span id (+ "down_" i!id)))))
        (author user i)
...
(def jitemline (i user)
  (when (cansee user i) 
    ;(when (news-type i) (jitemscore i user))
    (byline i user)))

(def jitemscore (i (o user))
  (tag (span id (+ "score_" i!id))
    (pr (plural (if (is i!type 'pollopt) (realscore i) i!score)
                "point")))
  (hook 'jitemscore i user))
...
(= title-limit* 80
   retry*       "Please try again. Note: URLs must begin with http:// or https://"
...
(newsop jsubmit ()
  (if user 
      (jsubmit-page user "" "" t) 
      (jsubmit-login-warning "" "" t)))

(def jsubmit-login-warning ((o url) (o title) (o showtext) (o text))
  (login-page 'both "You have to be logged in to submit."
              (fn (user ip) 
                (ensure-news-user user)
                (newslog ip user 'submit-login)
                (jsubmit-page user url title showtext text))))

(def jsubmit-page (user (o url) (o title) (o showtext) (o text "") (o msg))
  (minipage "Submit Job"
    (pagemessage msg)
    (urform user req
            (process-job (get-user req)
                           (clean-url (arg req "u"))
                           (striptags (arg req "t"))
                           showtext
                           (and showtext (md-from-form (arg req "x") t))
      ...
      (tab
        ...
        (row "" (submit))
        (spacerow 20)
        (row "" jsubmit-instructions*)))))

(= jsubmit-instructions*
   "Leave url blank to submit a job. If there is 
    no url, the text (if any) will appear at the top of the comments 
    page. If there is a url, the text will be ignored.")
...
(newsop jsubmitlink (u t)
  (if user 
      (jsubmit-page user u t)
      (jsubmit-login-warning u t)))
...
(= title-limit* 80
   ...
   toofast*     "You're submitting too fast.  Please slow down.  Thanks."
   spammage*    "Stop spamming us.  You're wasting your time."
   alumreq*     "You must be an alum to post a job.")
...
(def process-job (user url title showtext text ip)
  (aif (and (~blank url) (live-job-w/url url))
       (do (vote-for user it)
           (item-url it!id))
       (if (no user)
            (flink [jsubmit-login-warning url title showtext text])
           ;(no (alum user))
            ;(flink [jsubmit-page user url title showtext text alumreq*])
           (no (and (or (blank url) (valid-url url)) 
                    (~blank title)))
            (flink [jsubmit-page user url title showtext text retry*])
           (len> title title-limit*)
            (flink [jsubmit-page user url title showtext text toolong*])
           (and (blank url) (blank text))
            (flink [jsubmit-page user url title showtext text bothblank*])
           (let site (sitename url)
             (or (big-spamsites* site) (recent-spam site)))
            (flink [msgpage user spammage*])
           (oversubmitting user ip 'job url)
            (flink [msgpage user toofast*])
           (let s (create-job url (process-title title) text user ip)
             (job-ban-test user s ip url)
             (when (ignored user) (kill s 'ignored))
             (jsubmit-item user s)
             (maybe-ban-ip s)
             "jobs"))))

(def jsubmit-item (user i)
  (push i!id (uvar user submitted))
  (save-prof user)
  (vote-for user i))
...
(def create-job (url title text user ip)
  (newslog ip user 'create url (list title))
  (let s (inst 'item 'type 'job 'id (new-item-id) 
                     'url url 'title title 'text text 'by user 'ip ip)
    (save-item s)
    (= (items* s!id) s)
    (unless (blank url) (register-url s url))
...
(def job-ban-test (user i ip url)
  (site-ban-test user i url)
  (ip-ban-test i ip)
  (hook 'job-ban-test user i ip url))
...
(def news-type (i) (and i (in i!type 'story 'comment 'poll 'pollopt 'job)))
...
(= (displayfn* 'job)   (fn (n i user here inlist)
                           (display-job n i user here)))
...
(= (fieldfn* 'job)
   (fn (user b)
     (with (a (admin user)  e (editor user)  x (canedit user b))
       `((string1 title     ,b!title        t ,x)
         (url     url       ,b!url          t ,e)
         (mdtext2 text      ,b!text         t ,x)
         ,@(jstandard-item-fields b a e x)))))
...
(def jstandard-item-fields (i a e x)
       `((yesno   dead      ,i!dead        ,e ,e)
         (yesno   deleted   ,i!deleted     ,a ,a)
         (sexpr   flags     ,i!flags       ,a nil)
         (sexpr   keys      ,i!keys        ,a ,a)
